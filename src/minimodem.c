...
static void fsk_transmit_frame(simpleaudio *sa_out, unsigned int bits,
                               unsigned int n_data_bits, size_t bit_nsamples,
                               float bfsk_mark_f, float bfsk_space_f,
                               float bfsk_nstartbits, float bfsk_nstopbits,
                               int invert_start_stop, int bfsk_msb_first) {
  int i;
...
static void fsk_transmit_stdin(simpleaudio *sa_out, int tx_interactive,
                               float data_rate, float bfsk_mark_f,
                               float bfsk_space_f, int n_data_bits,
                               float bfsk_nstartbits, float bfsk_nstopbits,
                               int invert_start_stop, int bfsk_msb_first,
                               unsigned int bfsk_do_tx_sync_bytes,
                               unsigned int bfsk_sync_byte,
                               databits_encoder encode, int txcarrier) {
...
#else
  if (block_input) {
    fprintf(stderr, "E: This build of minimodem was configured without signal "
                    "support. Enable carrier?\n");
    exit(1);
  }
#endif
    ...
    if (!idle) {
      ...
      if (tx_transmitting < 2) {
        ...
        for (j = 0; j < bfsk_do_tx_sync_bytes; j++)
          fsk_transmit_frame(sa_out, bfsk_sync_byte, n_data_bits, bit_nsamples,
                             bfsk_mark_f, bfsk_space_f, bfsk_nstartbits,
                             bfsk_nstopbits, invert_start_stop, 0);
      }

      /* emit data bits */
      for (j = 0; j < nwords; j++)
        fsk_transmit_frame(sa_out, bits[j], n_data_bits, bit_nsamples,
                           bfsk_mark_f, bfsk_space_f, bfsk_nstartbits,
                           bfsk_nstopbits, invert_start_stop, bfsk_msb_first);
    } else {
      tx_transmitting = 1;
      /* emit idle tone (mark) */
      simpleaudio_tone(sa_out, invert_start_stop ? bfsk_space_f : bfsk_mark_f,
...
static void report_no_carrier(fsk_plan *fskp, unsigned int sample_rate,
                              float bfsk_data_rate, float frame_n_bits,
                              unsigned int nframes_decoded,
                              size_t carrier_nsamples, float confidence_total,
...
#endif
  float throughput_rate = nbits_decoded * sample_rate / (float)carrier_nsamples;
  ...
  if ((unsigned long long)(nbits_decoded * sample_rate + 0.5f) ==
    ...
    float throughput_skew = (throughput_rate - bfsk_data_rate) / bfsk_data_rate;
    fprintf(stderr, " (%.1f%% %s) ###\n",
...
static int benchmarks() {
#if USE_BENCHMARKS
  fprintf(stdout, "minimodem %s benchmarks\n", VERSION);

  int ret;
  ret = system(
      "sed -n -e '/^model name/{p;q}' -e '/^cpu model/{p;q}' /proc/cpuinfo");
  if (ret)
  ...
  sa_out = simpleaudio_open_stream(
      backend, NULL, SA_STREAM_PLAYBACK, SA_SAMPLE_FORMAT_S16, sample_rate, 1,
  ...
  sa_out = simpleaudio_open_stream(
      backend, NULL, SA_STREAM_PLAYBACK, SA_SAMPLE_FORMAT_FLOAT, sample_rate, 1,
...
void rx_stop_sighandler(int sig) { rx_stop = 1; }
...
int build_expect_bits_string(char *expect_bits_string, int bfsk_nstartbits,
                             int bfsk_n_data_bits, float bfsk_nstopbits,
...
#error At least one of {USE_SNDIO,USE_PULSEAUDIO,USE_ALSA,USE_SNDFILE,USE_STDIO} must be enabled!
  ...
  while (1) {
    ...
    c = getopt_long(argc, argv,
                    "Vtrc:l:ai875u:f:Ob:v:M:S:T:qs::A::R:", long_options,
                    &option_index);
    if (c == -1)
      break;
    switch (c) {
    ...
    case 'O':
      ...
      fprintf(
          stderr,
          "E: This build of minimodem was configured without stdio support.\n");
      exit(1);
#endif
      break;
    case '8':
    ...
    case 'A':
      ...
      fprintf(
          stderr,
          "E: This build of minimodem was configured without alsa support.\n");
      exit(1);
#endif
      break;
    case 's':
      ...
      fprintf(
          stderr,
          "E: This build of minimodem was configured without sndio support.\n");
      exit(1);
#endif
      break;
  ...
  if (filename) {
#if !USE_SNDFILE
    fprintf(stderr, "E: This build of minimodem was configured without "
  ...
  unsigned int bfsk_frame_n_bits =
      bfsk_n_data_bits + bfsk_nstartbits + bfsk_nstopbits;
  ...
  if (TX_mode) {
    ...
    simpleaudio *sa_out;
    sa_out = simpleaudio_open_stream(
        sa_backend, sa_backend_device, SA_STREAM_PLAYBACK, sample_format,
        sample_rate, nchannels, program_name, stream_name);
    if (!sa_out)
      return 1;

    fsk_transmit_stdin(sa_out, tx_interactive, bfsk_data_rate, bfsk_mark_f,
                       bfsk_space_f, bfsk_n_data_bits, bfsk_nstartbits,
                       bfsk_nstopbits, invert_start_stop, bfsk_msb_first,
                       bfsk_do_tx_sync_bytes, bfsk_sync_byte,
                       bfsk_databits_encode, txcarrier);
  ...
  simpleaudio *sa;
  sa = simpleaudio_open_stream(sa_backend, sa_backend_device, SA_STREAM_RECORD,
...
#define SAMPLE_BUF_DIVISOR 12
  ...
  if (expect_data_string == NULL) {
    ...
    expect_n_bits = build_expect_bits_string(
        expect_data_string, bfsk_nstartbits, bfsk_n_data_bits, bfsk_nstopbits,
        invert_start_stop, 0, 0);
  }
  debug_log("eds = '%s' (%lu)\n", expect_data_string,
            strlen(expect_data_string));
  ...
  while (1) {
    ...
    if (carrier_autodetect_threshold > 0.0f && carrier_band < 0) {
      ...
      for (i = 0; i + nsamples_per_scan <= samples_nvalid;
           i += nsamples_per_scan) {
        carrier_band =
            fsk_detect_carrier(fskp, samplebuf + i, nsamples_per_scan,
...
#define FSK_ANALYZE_NSTEPS 3
    ...
    confidence = fsk_find_frame(
        fskp, samplebuf, expect_nsamples, try_first_sample, try_max_nsamples,
        try_step_nsamples, try_confidence_search_limit,
        carrier ? expect_data_string : expect_sync_string, &bits, &amplitude,
        &frame_start_sample);
...
#define FSK_MAX_NOCONFIDENCE_BITS 20
    ...
    if (confidence <= fsk_confidence_threshold) {
      ...
      if (++noconfidence > FSK_MAX_NOCONFIDENCE_BITS) {
        ...
        if (carrier) {
          if (!quiet_mode)
            report_no_carrier(fskp, sample_rate, bfsk_data_rate, frame_n_bits,
...
#define FSK_ANALYZE_NSTEPS_FINE 8
        ...
        confidence2 = fsk_find_frame(
            fskp, samplebuf, expect_nsamples, try_first_sample,
            try_max_nsamples, try_step_nsamples, try_confidence_search_limit,
            carrier ? expect_data_string : expect_sync_string, &bits2,
    ...
    debug_log("@ confidence=%.3f peak_conf=%.3f amplitude=%.3f "
              "track_amplitude=%.3f\n",
    ...
    debug_log("@ nsamples_per_bit=%.3f n_data_bits=%u "
              ...
              nsamples_per_bit, bfsk_n_data_bits, frame_start_sample, advance);
    ...
    debug_log("Input: %08x%08x - Databits: %u - Shift: %i\n",
              (unsigned int)(bits >> 32), (unsigned int)bits, bfsk_n_data_bits,
              bfsk_nstartbits);
    ...
    dataout_nbytes += bfsk_databits_decode(dataoutbuf + dataout_nbytes,
                                           dataout_size - dataout_nbytes, bits,
